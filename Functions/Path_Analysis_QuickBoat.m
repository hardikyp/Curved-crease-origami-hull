function [PostprocessData] = Path_Analysis_QuickBoat(InputData, PreprocessData)
tol = 1e-6; 
...
%     b_lambda = InputData.loadMagnitude/InputData.numberIncrements;
    ...
    while icrm<MaxIcr && lmd < 1 % Analysis stops when applied load is below assigned load (\lambda < 1)
        ...
        if InputData.foldStep
            angles.pf0(PreprocessData.mountainFolds)=angles.pf0_Orig(PreprocessData.mountainFolds)
            angles.pf0(PreprocessData.valleyFolds)=[angles.pf0_Orig(PreprocessData.valleyFolds(1:InputData.numberDivisions)) + f_inc1 * icrm;
                                                    angles.pf0_Orig(PreprocessData.valleyFolds(InputData.numberDivisions+1:InputData.numberDivisions*2)) + f_inc2 * icrm;
                                                    angles.pf0_Orig(PreprocessData.valleyFolds(InputData.numberDivisions*2+1:InputData.numberDivisions*3)) + f_inc1 * icrm];
        end
        iter = 0; err = 1;
